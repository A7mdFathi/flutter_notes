...
class EditNotePage extends StatefulWidget {
  ...
  EditNotePage({Key key, Function() triggerRefetch, NoteModel existingNote})
      : super(key: key) {
...
class _EditNotePageState extends State<EditNotePage> {
  ...
  void handleSave() async {
    ...
    if (isNoteNew) {
      var latestNote = await NoteDatabase.db.addNoteInDB(currentNote);
      setState(() {
        currentNote = latestNote;
      });
    } else {
      await NoteDatabase.db.updateNoteInDB(currentNote);
    }
    setState(() {
      isNoteNew = false;
      isDirty = false;
                ...
                FlatButton(
                  child: Text('DELETE',
                      style: prefix0.TextStyle(
                          ...
                          letterSpacing: 1)),
                  onPressed: () async {
                    await NoteDatabase.db.deleteNoteInDB(currentNote);
                    widget.triggerRefetch();
                    Navigator.pop(context);
                    Navigator.pop(context);
                  },
